...
```{r}
SpeciesProx = ggplot(T_Manuals,aes(x = as.factor(RoundedWeekSinceRel),y = LC_Prox_m))+
      geom_boxplot(aes(fill=Species),outlier.color=NA)+
      #geom_boxplot(outlier.color=NA)+
      #coord_cartesian(ylim=c(0,10000))+
      scale_x_discrete(limits = as.character(1:20))+
      theme_classic(base_size=12)+
      scale_y_continuous(expand=c(0.013,0))+
      labs(x="Weeks Since Release",y="Proximity to Release Ramp (meters)")+
      theme(axis.text.x = element_text(angle = 45, hjust = 1))

BlackBassProx = ggplot(T_Manuals,aes(x = as.factor(RoundedWeekSinceRel),y = LC_Prox_m))+
      #geom_boxplot(aes(fill=Species),outlier.color=NA)+
      geom_boxplot(outlier.color=NA)+
      #coord_cartesian(ylim=c(0,10000))+
      scale_x_discrete(limits = as.character(1:20))+
...
ggsave(filename = "C:/Users/mrp0099/OneDrive - Auburn University/Desktop/ProposalGraphs/SpeciesProx.pdf",plot = SpeciesProx)
```
