...
TaggingMaster = TaggingMaster %>% 
            mutate(RewardLevel = case_when(
              Reward.Tag.. <= 1000 | Reward.Tag.. >=4000 & Reward.Tag..<=4499 ~ "Low",
              Reward.Tag.. >=1001 & Reward.Tag..<=2000 | Reward.Tag.. >=5000 & Reward.Tag..<=5249 ~ "Medium",
              Reward.Tag.. >=2001 & Reward.Tag..<=2500 ~ "High"
            ))
TaggingMaster
```

Combining the 2 together (single/double and the reward amount)
```{r}

TaggingMaster = TaggingMaster %>% 
              mutate(TaggingGroup = case_when(
                RewardLevel =="Low" & Num.tagged == 1 ~ "LowSin",
                RewardLevel == "Low" & Num.tagged == 2 ~ "LowDoub",
                RewardLevel == "Medium" & Num.tagged == 1 ~"MediumSin",
                RewardLevel == "Medium" & Num.tagged == 2 ~ "MediumDoub",
                RewardLevel == "High" ~ "HighSin"
              ))
...
Okay, we need first column is the tag type, second column is release period, second column is total tagged, and then every successive column is the recaps for each tagging period. 
            ...
            Rel Period  #Tagged 12/23   1/24   2/24   3/24   4/24   
...
```{r}
test = TaggingMaster %>% group_by(MonthYear, TaggingGroup) %>% count(n())
...
TagTypeOrder = c("LowSin","LowDoub","MediumSin","MediumDoub","HighSin")

test$MonthYear = factor(test$MonthYear, levels = MonthYearOrder)

test[with(test,order(factor(test$MonthYear, levels = MonthYearOrder),factor(test$TaggingGroup, levels = TagTypeOrder))),]
```




We may need to reorder it, but it looks like the above code calculated the correct number tagged for each tagging period and 
tag type. Now, we need to add-in the sequence of recap periods and how many recaptures were made from these groups every period. Firstly, we need to somehow merge the recapture data with the tagging master. This will allow us to desginate the tagging type of each of the returned fish. Then we can use the call data to assign it a recap period. 
