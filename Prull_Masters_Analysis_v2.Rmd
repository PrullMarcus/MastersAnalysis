...
First, we will look at manual tracking movements. There is an indicator column ("ManuallyTracked") that indicates whether we manually tracked a fish or not. For every fish the first location is the tagging location, followed by each subsequent location, therefore we need fish with >2 observations. 
```{r}
NT_Manuals = TrackMaster[TrackMaster$ManuallyTracked=="Y",]
NT_Manuals = NT_Manuals[NT_Manuals$TournamentRelease != "Y",]
NT_Manuals = NT_Manuals[NT_Manuals$GroupMovement == "A",]
NT_Manuals = NT_Manuals %>% group_by(RewardTag1) %>% filter (n()>2)
```
...
NeelyBuffer = terra::buffer(NeelyVec, width = 500)
...
```{r}
NT_Manuals = sf::st_as_sf(NT_Manuals, coords = c("Long","Lat"),crs = crs(NeelySHP))
```

Plotting of the data to make sure they show up together.
```{r}
plot(NeelyTransRast)
plot(NT_Manuals,add=T)
```
...
tag_nums = unique(NT_Manuals$RewardTag1)#unique tag numbers
n_fish = length(tag_nums)#number of fish

NT_Manuals_LC = numeric()
NT_Manuals_days = numeric()


for(i in 1:n_fish){
  
  new_tag = tag_nums[i]
  new_dat = subset(NT_Manuals,RewardTag1 == new_tag)
  tmax = nrow(new_dat)
...
```{r}
NT_Manuals = cbind(NT_Manuals_days,NT_Manuals)
colnames(NT_Manuals)[1]<-"DaysFromLastLoc"
NT_Manuals = cbind(NT_Manuals_LC,NT_Manuals)
colnames(NT_Manuals)[colnames(NT_Manuals) == "NT_Manuals_LC"] <- "LC_M"#renaming column to match Straight-line column name. 
NT_Manuals$LC_M_D = NT_Manuals$LC_M/NT_Manuals$DaysFromLastLoc
NT_Manuals$LC_M_W = NT_Manuals$LC_M/(NT_Manuals$DaysFromLastLoc/7)
```
...
```{r}
NT_Manuals$MonthYear = strftime(NT_Manuals$Date, format = "%Y-%m")
NT_Manuals = NT_Manuals %>% group_by(RewardTag1) %>% slice(3:n())
```

Filtering down to data I TOOK (2023 and on). 
```{r}
NT_Manuals = filter(NT_Manuals, Year %in% c(2023, 2024))
```

```{r}
#Boxplot comparing species movement over months. 
ggplot(NT_Manuals,aes(as.factor(MonthYear),LC_M_W))+
...
SeasonalPlot = ggplot(NT_Manuals,aes(x = MonthYear,y = LC_M_W))+
      geom_boxplot(aes(fill=Species),outlier.color=NA)+
      coord_cartesian(ylim=c(0,1500))+
      theme_classic(base_size=12)+
      scale_y_continuous(expand=c(0.013,0))+
      labs(x="Year-Month",y="Movement Speed (meters/week)")+
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
#ggsave(filename = "C:/Users/mrp0099/OneDrive - Auburn University/Desktop/ProposalGraphs/SeasonalMovement.pdf",plot = SeasonalPlot)
```
...
```{r}
months <- unique(NT_Manuals$MonthYear)

p_results <- data.frame(MonthYear = character(), p_value = numeric(), stringsAsFactors = FALSE)

for (m in months) {
  sub_data <- NT_Manuals[NT_Manuals$MonthYear == m, ]
  
  test_res <- wilcox.test(LC_M_W ~ Species, data = sub_data, exact = FALSE)
  
  p_results <- rbind(p_results, data.frame(MonthYear = m, p_value = test_res$p.value))
}
```
...
```{r}
Dates = make_date(year=T_Manuals$Year,month=T_Manuals$Month,day=T_Manuals$Day)
T_Manuals = cbind(Dates,T_Manuals)
colnames(T_Manuals)[1] <- "Date"
```
...
Movement Speeds as a function of time since Rel for both Black Bass and separated by species.
```{r}
BlackBassDispSpeed = ggplot(T_Manuals,aes(x = as.factor(RoundedWeekSinceRel),y = LC_M_W))+
      #geom_boxplot(aes(fill=Species),outlier.color=NA)+
      geom_boxplot(outlier.color=NA)+
      coord_cartesian(ylim=c(0,10000))+
      scale_x_discrete(limits = as.character(1:20))+
      theme_classic(base_size=12)+
      scale_y_continuous(expand=c(0.013,0))+
      labs(x="Weeks Since Release",y="Movement Speed (meters/week)")+
      theme(axis.text.x = element_text(angle = 45, hjust = 1))

SpeciesDispSpeed = ggplot(T_Manuals,aes(x = as.factor(RoundedWeekSinceRel),y = LC_M_W))+
      geom_boxplot(aes(fill=Species),outlier.color=NA)+
      coord_cartesian(ylim=c(0,10000))+
      scale_x_discrete(limits = as.character(1:20))+
      theme_classic(base_size=12)+
...
#ggsave(filename = "C:/Users/mrp0099/OneDrive - Auburn University/Desktop/ProposalGraphs/SpeciesDispSpeeds.pdf",plot = SpeciesDispSpeed)
```


```{r}
SpeciesProx = ggplot(T_Manuals,aes(x = as.factor(RoundedWeekSinceRel),y = LC_Prox_m/1000))+
      geom_boxplot(aes(fill=Species),outlier.color=NA)+
      #geom_boxplot(outlier.color=NA)+
      #coord_cartesian(ylim=c(0,10000))+
      scale_x_discrete(limits = as.character(1:20))+
      ...
      scale_y_continuous(expand=c(0.013,0))+
      labs(x="Weeks Since Release",y="Proximity to Release Ramp (kilometers)")+
      theme(axis.text.x = element_text(angle = 45, hjust = 1),
            axis.title.x = element_text(margin = margin(t=10)),
            axis.title.y = element_text(margin = margin(r = 10)),
            legend.position = c(0.9,0.9))

BlackBassProx = ggplot(T_Manuals,aes(x = as.factor(RoundedWeekSinceRel),y = LC_Prox_m/1000))+
      #geom_boxplot(aes(fill=Species),outlier.color=NA)+
      geom_boxplot(outlier.color=NA)+
...
ggsave(filename = "C:/Users/mrp0099/OneDrive - Auburn University/Desktop/ProposalGraphs/BlackBassProx.pdf",plot = BlackBassProx)
```


## Proportion of Alive Released Fish WITHIN 3 km Release Zone over time (Binomial Approach)
...
#### FISH ARE CONSIDERED DISPERSED INDEFINITELY
CL_Fish2 <- CL_Fish %>%
...
ggsave(filename = "C:/Users/mrp0099/OneDrive - Auburn University/Desktop/ProposalGraphs/TranslocateDistPlot.pdf",plot = TranslocateDistPlot)
```
...
#ggsave("PrePost_plot.pdf", plot = PrePost_plot)
```
...
u_index = numeric()
winter_fall_group = c(1,2,9,10,11,12,13,14,21,22,23,24)#what n_mon periods coorespond to the winter months (dec, jan, feb). 
u_counter = 2

for(i in 1:24) {
  if (i %in% winter_fall_group) {
    u_index[i] = 1
  } else {
    u_index[i] = u_counter
...
nll=function(theta,recaps,TL,nTag,RewardLevel,release_period){
  ...
  lambda=plogis(theta[4:5])
  lambda=c(lambda,1)
  
  u_param=plogis(theta[6:(5+13)])#should be 13 of these (sep, oct, nov, dec, jan, feb lumped into 1 u, separate for every other month)
  ...
  for (r in 1:n_rel){
    for(y in release_period[r]:n_mon){
      ...
      if (y %in% c(1:2,9:14,21:24)) {
        season = 1 #winter/fall
...
theta=c(qlogis(rep(0.3,3)),lambda=qlogis(c(.8,.9)), u = qlogis(rep(0.1,13)))
...
Here is a basic model looking at loading rate into the zone. But, firstly we need to get monthly dispersal rates post release. 

```{r}

dat = subset(TrackMaster, TournamentRelease =="Y" & ManuallyTracked == "Y")

dat = dat %>% group_by(RewardTag1) %>% filter(n()>1)


tag_nums = unique(dat$RewardTag1)
n_fish = length(tag_nums)# of individual fish making up dataset

dat_prox = numeric()
dat_months_since_rel = numeric()#only need to calculate this one time... So, this vector will not be included in the second loop

#Outside loop that selects tag-number 
for(i in 1:n_fish){
  
  new_tag = tag_nums[i]
  new_dat = subset(dat,RewardTag1 == new_tag)
  tmax = nrow(new_dat)
  dat_prox_loop = numeric(tmax)
  months = numeric(tmax)
  
  for(t in 2:tmax){
    
    dist_m = geosphere::distm(c(new_dat$Long[t],new_dat$Lat[t]),c(new_dat$Long[1],new_dat$Lat[1]))
    dat_prox_loop[t] = dist_m
    months[t] = as.numeric(new_dat$Date[t]-new_dat$Date[1])/30.44
  }
  dat_prox = append(dat_prox,dat_prox_loop)
  dat_months_since_rel = append(dat_months_since_rel,months)
}

dat$ProxToRamp = dat_prox
dat$MonthsSinceRel = dat_months_since_rel

```

```{r}

dat$floor_month = floor(dat$MonthsSinceRel)
dat$ceiling_month = ceiling(dat$MonthsSinceRel)
dat$rounded_month = round(dat$MonthsSinceRel)

#dat = subset(dat,GroupMovement == "A")


proportional_montly_out = dat %>% group_by(floor_month) %>%
                      summarize(
                      total_fish = n_distinct(RewardTag1),
                      fish_near_ramp = n_distinct(RewardTag1[ProxToRamp <=3000])
                          ) %>% 
                      mutate(prop_near_ramp = fish_near_ramp/total_fish)

proportional_montly_out_rounded_month = dat %>% group_by(rounded_month) %>%
                      summarize(
                      total_fish = n_distinct(RewardTag1),
                      fish_near_ramp = n_distinct(RewardTag1[ProxToRamp <=3000])
                          ) %>% 
...
interpolated <- dat %>%
  dplyr::select(RewardTag1, MonthsSinceRel, ProxToRamp) %>%
  group_by(RewardTag1) %>%
  arrange(MonthsSinceRel) %>%
  summarise(
    interp = list(approx(
      x = MonthsSinceRel,
      y = ProxToRamp,
  ...
  unnest_wider(interp) %>%
  unnest(cols = c(x, y)) %>%
  rename(month_bin = x, proximity_interp = y)

interpolated %>% filter(!is.na(proximity_interp))

proportional_in = interpolated %>% group_by(month_bin) %>%
                      summarize(
                      total_fish = n_distinct(RewardTag1),
                      fish_near_ramp = n_distinct(RewardTag1[proximity_interp <=3000])
                          ) %>% 
                      mutate(prop_near_ramp = fish_near_ramp/total_fish)
...
plot(st_geometry(alabama))
plot(studyArea, col='black', add=T)

 zoomedOut = ggplot()+
  geom_sf(data = alabama, fill = NA, color = "black")+
  geom_sf(data = studyArea, fill = "black", color='black')

zoomedOut = zoomedOut +
  theme_void()+ #Gets rid of most of the colors
...
```{r}
#Adding column to shp files that legend labels will use
ReleaseZone$ZoneLabel = "Release Zone"
CL_SHP$Label = "Coosa Landing Boat Launch"

bbox_studyarea = st_bbox(NeelySHP)

NeelyPlot = ggplot() + 
  geom_sf(data = NeelySHP, fill = "NA", color = "black") +
  geom_sf(data = ReleaseZone, aes(fill = ZoneLabel)) + 
  geom_sf(data = CL_SHP, aes(shape = Label, color = Label), size = 3, fill = "red") + 
  
  scale_fill_manual(name = NULL, values = c("Release Zone" = "yellow"))+
  scale_shape_manual(values = c("Coosa Landing Boat Launch" = 24),name=NULL)+
  scale_color_manual(values = c("Coosa Landing Boat Launch" = "black"),name = NULL)+
  ...
  coord_sf(xlim = c(bbox_studyarea["xmin"],bbox_studyarea["xmax"]),
           ylim = c(bbox_studyarea["ymin"],bbox_studyarea["ymax"]),
          expand = FALSE ) +
...
NeelyMap = ggdraw() +
 draw_plot(NeelyPlot)+
 draw_plot(zoomedOut, x = 0.07, y = 0.84, width = 0.15, height = 0.15)
NeelyMap

ggsave("NeelyOutline2.pdf", plot = NeelyMap,width = 7, height = 7, units = "in", dpi = 400, device=cairo_pdf)
```
