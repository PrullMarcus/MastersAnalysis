...
## Homerange Analysis MIGHT NOT BE POSSIBLE (TOO SMALL OF SAMPLE SIZES)
...
```{r}
NT_HR = subset(TrackMaster, GroupMovement == "A")#only alive fish
NT_HR = subset(NT_HR, TournamentRelease != "Y")#only non-tournament fish
NT_HR = subset(NT_HR, ManuallyTracked == "Y")#only fish that were manually tracked
...
```{r}
NT_HR = NT_HR %>% group_by(RewardTag1) %>% filter(n()>20)
NT_HR = as(NT_HR, "Spatial")
NT_mcp95 = mcp(NT_HR[,"RewardTag1"],percent = 95, unin="m", unout="m2")# is unit in correct?????
...
Here is the actual model that we are going to use. 

Is this simulating the overall tag returns or specifically fish caught in tournaments by period (month). 
```{r}
rm(list=ls(all=TRUE))
set.seed(1)

n_tag_1 = #number tagged per release event
n_tag_2 = #release event 2
n_tag_3 = #release event 3
...
n_rel=3# I assume 3 due to 3 tag release events? 
n_yrs=#????

u=runif(n_yrs,0.1,0.4)#random exploitation rate (do we know this)
lambda=0.6#percent of low dollar tags that get reported?????
cm=0.2#fish that die after fishing?
TL=0.05#tag loss?

S=matrix(NA,nrow=n_rel,ncol=n_yrs)#???

p_ret_hi=matrix(0,nrow=n_rel,ncol=n_yrs+1)
p_ret_med=matrix(0,nrow=n_rel,ncol=n_yrs+1)
p_ret_low=matrix(0,nrow=n_rel,ncol=n_yrs+1)

#Hi = 300, Med = 200, Low = 100 
recaps_hi=matrix(0,nrow=n_rel,ncol=n_yrs+1)
recaps_med=matrix(0,nrow=n_rel,ncol=n_yrs+1)
recaps_low=matrix(0,nrow=n_rel,ncol=n_yrs+1)

for (r in 1:n_rel){
  for(y in r:n_yrs){
    S[r,y]=(1-u[y])*(1-cm)
    if(r==y){
      p_ret_hi[r,y]=u[y]*(1-TL)
      p_ret_low[r,y]=u[y]*(1-TL)*lambda
    } else {
      p_ret_hi[r,y]=u[y]*(1-TL)*prod(S[r,r:(y-1)])
      p_ret_low[r,y]=u[y]*(1-TL)*lambda*prod(S[r,r:(y-1)])
    }  # close if else
    
  }  #close y loop
  p_ret_hi[r,n_yrs+1]=1-sum(p_ret_hi[r,])
  p_ret_low[r,n_yrs+1]=1-sum(p_ret_low[r,])
  recaps_hi[r,]=rmultinom(1,size=n_tag_hi,prob=p_ret_hi[r,])
  recaps_low[r,]=rmultinom(1,size=n_tag_low,prob=p_ret_low[r,])
}  #close r loop
...
nll=function(theta,recaps_hi,recaps_low,TL){
  cm=plogis(theta[1])
  lambda=plogis(theta[2])
  u=plogis(theta[3:(n_yrs+2)])
  S=matrix(NA,nrow=n_rel,ncol=n_yrs)
  
  p_ret_hi=matrix(0,nrow=n_rel,ncol=n_yrs+1)
  p_ret_low=matrix(0,nrow=n_rel,ncol=n_yrs+1)
  
  nll_hi=numeric(n_rel)
  nll_low=numeric(n_rel)
  ...
  for (r in 1:n_rel){
    ...
    p_ret_hi[r,n_yrs+1]=1-sum(p_ret_hi[r,])
    p_ret_low[r,n_yrs+1]=1-sum(p_ret_low[r,])
    nll_hi[r]=-dmultinom(recaps_hi[r,],prob=p_ret_hi[r,],log=T)
    nll_low[r]=-dmultinom(recaps_low[r,],prob=p_ret_low[r,],log=T)
  } #close r loop
  sum(nll_hi)+sum(nll_low)
}  #close function

theta=qlogis(c(cm,lambda,u))

nll(theta,recaps_hi,recaps_low,TL)
