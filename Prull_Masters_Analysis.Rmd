...
```{r}
ggplot(ControlFinal,aes(as.factor(YearMonth),SL_M_D))+
...
```{r}
NeelyVec = vect(Neely)
NeelyR = rast(NeelyVec, res = 0.00011)#res of 11.1 m grid cells, value of 1 is 1 "degree". that is why we need to use a big decimal to get it to meters. tried 00004 but it took way too long. 
...
Control = sf::st_as_sf(Control, coords = c("Lat","Long"),crs = crs(Neely))#Change to control(adds geometry but wont get rid of anything).
...
#### FILTER EVERYTHING AT THE END INSTEAD OF DOING THE INITIAL FILTER FOR THE SL PART!!!!!
